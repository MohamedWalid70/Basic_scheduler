...
TaskAdditionState_t SCHED_enumAddTask(void (*copy_pvTaskFunc)(void*), uint16 copy_uint16Periodicity, uint8 copy_uint8Priority, uint8 copy_uint8TaskID){
	...
	for(uint8 i=0;i<SCHED_TASKS_NUMBER;i++){
		...
		if(private_arrTask_tSysTasks[i].taskFunc != NULL){
			...
			if(private_arrTask_tSysTasks[i].taskState == ready){
				...
				private_arrTask_tSysTasks[i].taskFunc(NULL);
				private_arrTask_tSysTasks[i].temp=private_arrTask_tSysTasks[i].periodicity-SCHED_TICK_TIME_IN_MILLI_SEC;
				private_arrTask_tSysTasks[i].taskState = waiting;

			}else if(private_arrTask_tSysTasks[i].taskState == waiting){

				private_arrTask_tSysTasks[i].temp-=SCHED_TICK_TIME_IN_MILLI_SEC;

				if(private_arrTask_tSysTasks[i].temp==0){
